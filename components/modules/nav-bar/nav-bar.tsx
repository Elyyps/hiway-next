...
const NavBarComponent = (props: INavBarComponentProps) => {
  ...
  return (
    <div className={style["nav-bar"]}>
      <div className={` ${style["nav-bar-container"]} container`}>
        <div
          className={style["nav-bar-logo"]}
          onClick={handleLogoClick}
          style={{ height: windowSize > 639 ? windowSize / 28 : "" }}
        >
        ...
        <div className={` ${style["nav-bar-menu"]} ${"uk-visible@s"} `}>
          <ul>
            {props.navBarModule.items.map((item, key) => (
              ...
              >
                {item.dropdownLink ? (
                  <DropdownComponent title={item.title.text} fullWidth>
                    <div className={style["nav-bar-dropdown"]}>
                      <div className="uk-container uk-container-small">
                        <div className={style["dropdown-left"]}>
                          <span>
                            {item.dropdownTitle && item.dropdownTitle}
                          </span>
        ...
        <div className={`${style["nav-bar-language"]}`}>
          ...
          <div
            className={`${style["nav-bar-language-desktop"]} ${style["large"]} uk-visible@s `}
          >
            <span>Language :</span>
            <DropdownComponent
      ...
      <div
        className={`${style["nav-bar-language-desktop"]} ${style["x-large"]}`}
      >
        <span>Language :</span>
        <DropdownComponent
          title={language === "EN" ? "English" : "Dutch"}
